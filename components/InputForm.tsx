...
function GeneratingState() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
      {/* Spinner minimalista */}
      <div className="w-10 h-10 border-2 border-foreground/20 border-t-foreground rounded-full animate-spin mb-10" />
...
export function InputForm({ onGenerate }: InputFormProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      console.error(err);
      setError("Ocurrió un error al generar. Revisá tu descripción e intentá de nuevo.");
      setLoading(false);
    }
  };
  ...
  return (
    <Card className="p-8 max-w-2xl mx-auto w-full">
      ...
      <p className="text-muted-foreground mb-8">
        Subí tu logo, algunas referencias y describí tu marca. Generamos tu brandbook completo.
      </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          <Label htmlFor="logo">
            Logo{" "}
            <span className="text-muted-foreground font-normal">(PNG, JPG, SVG — opcional)</span>
          </Label>
          <Input
            id="logo"
            type="file"
          ...
          />
          {logo && <p className="text-sm text-muted-foreground mt-1">✓ {logo.name}</p>}
        </div>

        <div>
          <Label htmlFor="references">
            Referencias visuales{" "}
            <span className="text-muted-foreground font-normal">(hasta 4 imágenes — opcional)</span>
          </Label>
          <Input
            id="references"
            type="file"
          ...
          {references.length > 0 && (
            <p className="text-sm text-muted-foreground mt-1">
              ✓ {references.length} imagen{references.length > 1 ? "es" : ""} seleccionada
        ...
        <div>
          <Label htmlFor="prompt">Describí tu marca *</Label>
          <Textarea
            id="prompt"
            {...register("prompt")}
            placeholder="Ej: Somos una agencia de diseño para startups de tecnología. Nuestro tono es profesional pero cercano, innovador pero accesible. El público son founders de 28 a 40 años..."

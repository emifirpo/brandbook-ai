    ...
    <div className="min-h-screen bg-background py-12 px-4">
      <div className="max-w-[var(--layout-page-max-width)] mx-auto">
        <header className="mb-12">
          <p className="text-xs font-medium tracking-widest uppercase text-muted-foreground mb-3">Brandbook</p>
          <h1 className="text-5xl font-bold tracking-tight mb-2">{brandbook.brand_name}</h1>
        ...
        <div className="space-y-[var(--layout-section-gap)]">
          <BrandStorySection brandStory={brandbook.brand_story} brandbook={brandbook} onUpdate={(c) => updateSection("brand_story", c)} />
          <VisualIdentitySection visualIdentity={brandbook.visual_identity} brandbook={brandbook} onUpdate={(C) => updateSection("visual_identity", C)} />
          <LogoUsageSection logoUsage={brandbook.logo_usage} brandbook={brandbook} onUpdate={(c) => updateSection("logo_usage", c)} />
          <ToneOfVoiceSection toneOfVoice={brandbook.tone_of_voice} brandbook={brandbook} onUpdate={(C) => updateSection("tone_of_voice", C)} />
          <TargetAudienceSection targetAudience={brandbook.target_audience} brandbook={brandbook} onUpdate={(C) => updateSection("target_audience", C)} />
          <AiPromptPackSection aiPromptPack={brandbook.ai_prompt_pack} />
        </div>
      </div>
    </div>
  );
